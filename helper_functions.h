...
/*
static double computeMetric(std::vector<double> vec1, std::vector<double> vec2){
	...
	for(int i = 0; i < vec1.size(); i++){
		metric += std::pow(vec1[i] - vec2[i], 2);
	}
	return metric;
}
*/
/*
static double computeMetric(std::vector<double> vec1, std::vector<int> vec2){
	if(vec1.size() != vec2.size()){
		std::cout << "vector size mismatch" << std::endl;
		return -1;
	}
