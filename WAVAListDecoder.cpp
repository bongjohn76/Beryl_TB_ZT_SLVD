...
std::vector<std::vector<ListDecoder::cell>> ListDecoder::constructWAVATrellis(std::vector<double> receivedMessage, std::vector<std::vector<cell>> oneTrellis){
	std::vector<std::vector<cell>> trellisInfo;
	trellisInfo = std::vector<std::vector<cell>>(numStates, std::vector<cell>(pathLength));

	// initializes all the valid starting states
	for(int i = 0; i < numStates/2; i++){
		trellisInfo[i][0].pathMetric = oneTrellis[i][oneTrellis[i].size() - 1].pathMetric;
...
ListDecoder::messageInformation ListDecoder::modifiedWAVADecoding(std::vector<double> receivedMessage){
	
	pathLength = receivedMessage.size() + 1;

	// trellisInfo is indexed [state][stage]
	std::vector<std::vector<cell>> trellisInfo;
    std::vector<std::vector<cell>> oneTrellis = constructOneTrellis(receivedMessage);
	messageInformation output;
	
    std::vector<int> path(pathLength);
    int endingStateMetric = INT_MAX;
    for(int i = 0; i < numStates / 2; i++){
		if(oneTrellis[i][pathLength - 1].pathMetric < endingStateMetric){
            endingStateMetric = oneTrellis[i][pathLength - 1].pathMetric;
            path[pathLength - 1] = i;
        }
    }
    for(int i = pathLength - 1; i > 0; i--)
        path[i - 1] = oneTrellis[path[i]][i].optimalFatherState;
	...
	trellisInfo = constructWAVATrellis(receivedMessage, oneTrellis);
...
ListDecoder::messageInformation ListDecoder::wavaDecoding(std::vector<double> receivedMessage){
	pathLength = receivedMessage.size() + 1;
