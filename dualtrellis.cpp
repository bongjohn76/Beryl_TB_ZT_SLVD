	...
	:numInputSymbols(2), numOutputSymbols(1)
{
	...
	this->subsequentStates = std::vector<std::vector<std::vector<int>>>(Nstates, std::vector<std::vector<int>>(n, std::vector<int>(2)));
	this->prevStates = std::vector<std::vector<std::vector<int>>>(Nstates, std::vector<std::vector<int>>(n, std::vector<int>(2, -1)));
			...
			for (int cur_state = 0; cur_state < (Nstates); cur_state++){
				if (cur_valid_states[cur_state] == 1){
					...
					for (int input = 0; input < (numInputSymbols - 1); input++){
						std::vector<int> next_state_bin(v);
						for (int p = 0; p < v; p++){
							next_state_bin[p] = (cur_state_bin[p] + (cur_state_bin[v - 1] * H[i][p])) % 2;
						}
...
int DualTrellis::getV(){
	return v;
}

int DualTrellis::getN(){
	return n;
}