...
FeedbackTrellis::FeedbackTrellis(int k, int n, int v, std::vector<int> numerators, int denominator){
	this->k = k;
	this->n = n;
	this->v = v;
	...
	if(v != V){
		std::cout << "MAJOR ISSUE: CONST V DOES NOT MATCH v. EDIT IN FEEDBACKTRELLIS" << std::endl;
		exit(1);
	}
	computeNextStates();
	computeTerminations();
...
void FeedbackTrellis::computeNextStates(){
	...
	for (int currentState = 0; currentState < numStates; currentState++){
		...
		for (int input = 0; input < numInputSymbols; input++){
			...
			std::cout << "total numerator: " << std::endl;
			for(int i = 0; i < V + 1; i++){
				std::cout << total_numerator[i] << "\t";
			}
			std::cout << std::endl;
			...
			galois::GaloisField gf1 = galois::GaloisField(V, b_arr);
			...
			galois::GaloisFieldPolynomial poly1 = galois::GaloisFieldPolynomial(&gf1, V, gfe1);
			galois::GaloisFieldPolynomial poly2 = galois::GaloisFieldPolynomial(&gf1, V, gfe2);
			...
			// VERBOSE DEBUGGING: USED TO CHECK EQUALITY BETWEEN MY GFDIV AND PROPER GFDIV 
			std::cout << std::endl << "remd: " << std::endl;
			for(int i = 0; i < remd.size(); i++){
				std::cout << remd[i] << "\t";
			}
			...
			// CHECKS EQUALITY BETWEEN GALOIS FIELD LIBRARY AND MY IMPLEMENTATION- SHOULD BE INCLUDED UNTIL
			// EQS ARE VALIDATED ACROSS ALL RELEVENT CODES
			if(V != v){
				std::cout << "constant V does not match- check feedbacktrellis" << std::endl;
			}
			...
			for (int i = (next_sigmas.size() - 1); i >= 0; i--){
				nextState += next_sigmas[i] * pow(2, (next_sigmas.size() - 1 - i)); //convert next_sigmas back into decimal value
			}
...
std::vector<std::vector<int>> FeedbackTrellis::getOutputs(){
	return outputs;
}

int FeedbackTrellis::getNumInputSymbols(){
	return numInputSymbols;
}

int FeedbackTrellis::getNumOutputSymbols(){
	return numOutputSymbols;
}

int FeedbackTrellis::getNumStates(){
	return numStates;
}
...
void FeedbackTrellis::computeTerminations(){
    ...
    while (!queue.empty()){
        ...
        for (int i = 0; i < numStates; i++){
            if (std::count(nextStates[i].begin(), nextStates[i].end(), target_state) > 0 && vis[i]==0){
                vis[i] = 1;
                queue.push(i);
                tree[i] = target_state;
            }
	...
	terminations = std::vector<std::vector<int>>(numStates, std::vector<int>(num_transitions));
    ...
    for (int i = 1; i < numStates; i++){
        ...
        for (int j = 0; j < num_transitions; j++){
            ...
            int index = std::distance(nextStates[cur].begin(), std::find(nextStates[cur].begin(), nextStates[cur].end(), fa_state));
			terminations[i][j] = index;
            cur = fa_state;
        }
    }
	std::cout << "made it to the end of the termination computations" << std::endl;
}

std::vector<int> FeedbackTrellis::terminateMsg(std::vector<int> originalMessage){
...
std::vector<int> FeedbackTrellis::ztencoder(std::vector<int> terminatedMessage){
